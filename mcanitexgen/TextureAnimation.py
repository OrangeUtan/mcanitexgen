...
class TextureAnimation:
    ...
    @classmethod
    def from_json(
        cls, name: str, json: dict, texture_animations: Dict[str, TextureAnimation] = dict()
    ) -> TextureAnimation:
        # Parse constants
        constants = dict()
        ...
        for entry_name, entries in json.get("sequences", {}).items():
            sequence = Sequence.from_json(
                entry_name,
                entries,
                states.keys(),
                sequence_names,
                {**texture_animations, **constants},
            )
    ...
    def mark(self, mark_name: str, index: int = 0):
        if not mark_name in self.marks:
            raise MCAnitexgenException(
                f"TextureAnimation '{self.name}' doesn't have mark '{mark_name}'"
            )
    ...
    def validate(self):
        if self.entries:
            if not self.start == self.entries[0].start:
                raise MCAnitexgenException(
                    f"AnimatedGroup does not start with its first entry: {self}'"
                )
            if not self.end == self.entries[-1].end:
                raise MCAnitexgenException(
                    f"AnimatedGroup does not end with its last entry: {self}'"
                )
...
class Sequence:
    ...
    total_weight: int
    fixed_duration: int = field(
        default=None, init=False
    )  # Duration of the sequence unaffected by weight. Can be thought of as a 'minimum' duration
    ...
    def calc_fixed_duration(self, sequences: Dict[str, Sequence]) -> int:
        ...
        if not self.fixed_duration:
            self.fixed_duration = sum(
                map(lambda entry: entry.calc_fixed_duration(sequences), self.entries)
            )
    ...
    def to_animation(
        self, currentTime: int, duration: int, textureAnimation: TextureAnimation
    ) -> AnimatedGroup:
        # Are there any weighted entries
        if self.is_weighted:
            if duration == 0:
                raise MCAnitexgenException(
                    f"Didn't pass duration to weighted sequence '{self.name}'"
                )
            if duration <= self.fixed_duration:
                raise MCAnitexgenException(
                    f"Sequence '{self.name}': Duration must be at least '{self.fixed_duration}', but was '{duration}'"
                )

            # Calculate times for weighted entries
            distributable_duration = (
                duration - self.fixed_duration
            )  # Duration that can be distributed over the weighted entries
            weights_of_weighted_entries = map(
                lambda entry: entry.weight,
                filter(lambda entry: entry.has_weight, self.entries),
            )
            weighted_durations = partition_by_weights(
                distributable_duration, self.total_weight, weights_of_weighted_entries
            )
        ...
        for i, entry in enumerate(self.entries):
            ...
            if entry.start:
                if currentTime == 0:
                    raise MCAnitexgenException(
                        f"Sequence '{self.name}': {i+1}. entry can't start at '{entry.start}', because there is no previous entry"
                    )
                if currentTime > entry.start:
                    raise MCAnitexgenException(
                        f"Sequence '{self.name}': {i+1}. entry can't start at '{entry.start}', because of previous entry"
                    )
            ...
            if entry.end:
                if currentTime >= entry.end:
                    raise MCAnitexgenException(
                        f"Sequence '{self.name}': {i+1}. entry can't end at '{entry.end}', because of previous entry"
                    )
            ...
            if entry.has_weight:
                ...
                repetition_durations = partition_by_weights(
                    entry_duration,
                    entry.weight * entry.repeat,
                    itertools.repeat(entry.weight, entry.repeat),
                )
            else:
            ...
            for repetition_duration in repetition_durations:
                if repetition_duration <= 0:
                    raise MCAnitexgenException(
                        f"Sequence '{self.name}': Duration of '{distributable_duration}' exhausted while trying to distribute it over entries"
                    )

                # Convert to AnimatedEntry
                animatedEntry = entry.to_animated_entry(
                    currentTime, repetition_duration, textureAnimation
                )
                ...
                # Add mark if any
                if entry.mark:
                    textureAnimation.add_mark(
                        entry.mark, AnimationMark(animatedEntry.start, animatedEntry.end)
                    )
...
class SequenceEntry:
    ...
    def from_json(cls, json: Dict, expr_locals: Dict[str, Any]) -> SequenceEntry:
        if str(SequenceEntryType.SEQUENCE) in json:
            ...
            raise MCAnitexgenException(
                "Sequence entry is missing reference to state or sequence"
            )
        ...
        try:
            start = (
                int(evaluate_expr(str(json.get("start")), expr_locals))
                if "start" in json
                else None
            )
        except Exception as e:
        ...
        try:
                ...
                int(evaluate_expr(str(json.get("end")), expr_locals))
                if "end" in json
                else None
            )
        except Exception as e:
    ...
    def to_animated_entry(
        self, start: int, duration: int, textureAnimation: TextureAnimation
    ) -> AnimatedEntry:
        if self.type == SequenceEntryType.STATE:
            return AnimatedState(
                start, start + duration, textureAnimation.states[self.ref].index
            )
        else:
            return textureAnimation.sequences[self.ref].to_animation(
                start, duration, textureAnimation
            )

    def validate_reference(
        self, parent_sequence: str, state_names: List[str], sequence_names: List[str]
    ):
    ...
    def calc_fixed_duration(self, sequences: Dict[str, Sequence]) -> int:
        ...
        if self.type == SequenceEntryType.STATE:
            ...
            if not sequences[self.ref].is_weighted:
                fixed_duration = (
                    sequences[self.ref].calc_fixed_duration(sequences) * self.repeat
                )
    ...
    def __init__(self, parent_sequence: str, ref_target: str, ref_type: SequenceEntryType):
        if ref_type == SequenceEntryType.STATE:
            super(InvalidReferenceException, self).__init__(
                f"Reference '{ref_target}' in sequence '{parent_sequence}' does not name a state"
            )
        elif ref_type == SequenceEntryType.SEQUENCE:
            super(InvalidReferenceException, self).__init__(
                f"Reference '{ref_target}' in sequence '{parent_sequence}' does not name a sequence"
            )
